...
class PacketParserTestCase(unittest.TestCase):
    def test_usnd_packet_parser(self):
        packet_data = packet_parser.parse_raw_data(packet_structure='<BIIB',
    ...
    def test_usnd_packet_parser_incomplete(self):
        self.assertRaises(packet_parser.ParserBadChecksum, packet_parser.parse_raw_data,
                          packet_structure='<BIIB',
                          raw_data=b'd\xc3\x89\x00')

    def test_status_packet_parser(self):
        packet_data = packet_parser.parse_raw_data(packet_structure='<BIIHHHB',
